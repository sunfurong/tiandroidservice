...
#define TAG "cn.named.service"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Androidservice_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Androidservice.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::AndroidserviceBindings::lookupGeneratedInit(
...
static void Androidservice_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Androidservice_getBinding)->GetFunction());
}

static void Androidservice_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::AndroidserviceBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry AndroidserviceBinding = {
	"cn.named.service",
	Androidservice_init,
	Androidservice_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_cn_named_service_AndroidserviceBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("cn.named.service", &AndroidserviceBinding);
	titanium::KrollBindings::addExternalLookup(&(::AndroidserviceBindings::lookupGeneratedInit));
}
